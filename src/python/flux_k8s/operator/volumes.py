...
class VolumeManager:
    ...
    def create_persistent_volume(self, rabbits, storage_capacity=None):
        ...
        try:
            ...
            if e.reason == "AlreadyExists":
                print("PV already exists. No action taken.")
            else:
                print(f"Error creating PersistentVolume {self.name}': {e}")
                raise
    ...
    def delete_persistent_volume_claim(self):
        ...
        try:
            ...
            if e.status != 404:
                print(f"Error deleting PersistentVolumeClaim '{self.name}': {e}")
                raise
    ...
    def delete_persistent_volume(self):
        ...
        try:
            ...
            if e.status != 404:
                print(f"Error deleting PersistentVolume '{self.name}': {e}")
                raise
